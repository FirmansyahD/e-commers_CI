...
<div class="col-md-6">
	<table id="example" class="table table-hover table-striped">
		...
		<tbody>
			<?php $no=0; foreach($data_motor as $motor): $no++; ?>
			<tr>
				<td><?=$no?></td>
				...
				<?php if($motor->stok>0): ?>
				<td><a href="<?=base_url('index.php/transaksi/addcart/'.$motor->id_motor)?>" class="btn btn-info">Pesan</a></td>
			<?php elseif($motor->stok==0): ?>
...
<div class="col-md-6">
	<form action="<?=base_url('index.php/transaksi/simpan')?>" method="post">
		...
		Nama Pembeli : <select name="id_pembeli"selected >
                
                <?php foreach($AllDataPelanggan as $pelanggan): ?>
                <option value="<?=$pelanggan->id_pembeli?>"><?=$pelanggan->nama_pembeli?></option>
                <?php endforeach ?>
              </select><br>
		...
		<table class="table table-striped table-hover" blocked>
			<tr>
				...
				<input type="hidden" name="id_motor[]" value="<?=$items['id']?>">
